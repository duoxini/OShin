...
// 获取当前 Git 提交的短哈希值。
val gitCommitHashProvider = if (isGitRepo) {
    providers.exec {
        commandLine("git", "rev-parse", "--short", "HEAD")
    }.standardOutput.asText.map { it.trim() }
} else {
    providers.provider { "dev" }
}

// 获取从项目起始到当前 HEAD 的总提交次数。
val gitCommitCountProvider = if (isGitRepo) {
    providers.exec {
...
android {
    ...
    defaultConfig {
        ...
        multiDexEnabled = false           // 禁用 Multidex 以防止生成多个 Dex 文件
    ...
    signingConfigs {
        ...
        // 移除空的 "release" 签名配置，以避免 "missing required property 'storeFile'" 错误。
        // 在本地构建时，将默认使用 debug 签名。
    }
    ...
    buildTypes {
        release {
            val keystoreFile = System.getenv("KEYSTORE_PATH")
            val isCiBuild = keystoreFile != null
            // 如果是 CI 环境（配置了 KEYSTORE_PATH），则使用 ci 签名配置。
            // 否则（本地开发），使用 debug 签名配置，确保能够正常编译和安装。
            signingConfig = if (isCiBuild) signingConfigs.getByName("ci") else signingConfigs.getByName("debug")
